...
def format_description_field(text):
    """
    get first 100 words from text
    """
    text_formated = format_string(text) #first format raw text
    splitted_text = text_formated.split()
    first_60_words = splitted_text[:60]
    return ' '.join(first_60_words)  # join them back into a string
...
def scrap_books_attribute(base_link):
    ...
    """
    formating list values needed 
    """
    _final_list= {}

    _final_list['DT_PROCESS_DATE']= str(datetime.datetime.now())
    _final_list['DS_BOOK_CATEGORY']= _temp_list[4]
    _final_list['DS_BOOK_TITLE']= format_string(_temp_list[6])
    _final_list['NR_BOOK_PRICE']= replace_monetary(_temp_list[7])
    _final_list['DS_BOOK_DESCRIPTION']= format_description_field(_temp_list[11])
    _final_list['NR_UPC_NUMBER']= str(_temp_list[13]).replace('UPC','')
    _final_list['TP_PRODUCT_TYPE']= str(_temp_list[14]).replace('Product Type','').lower()
    _final_list['NR_NO_TAX_PRICE']= replace_monetary(_temp_list[15])
    _final_list['NR_W_TAX_PRICE']= replace_monetary(_temp_list[16])
    _final_list['NR_TOTAL_AVAILABLE']= format_available_field(_temp_list[19])
    _final_list['DS_BOOK_STAR']= scrap_star_rating(_html_code_filtered)

    """
    get only values from dict and trasnform in tuple
    """
    formated_tupled_data = tuple(_final_list.values())

    return formated_tupled_data
